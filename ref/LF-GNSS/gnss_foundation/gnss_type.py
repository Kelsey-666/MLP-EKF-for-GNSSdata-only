...
class uSIG(IntEnum):
    ...
    # GAL  E5a+E5b 1191.795 MHz
    # BDS  B2a+B2b 1191.795 MHz
    L8 = 800
    L8D = 804
    L8I = 809
    L8P = 816
...
class rSigRnx():
    ...
    def str2sig(self, sys, s):
        ...
        if s[0] == 'C':
            self.typ = uTYP.C  # Code pseudorange
        elif s[0] == 'L':
            self.typ = uTYP.L  # Carrier phase
        elif s[0] == 'D':
            self.typ = uTYP.D  # Doppler frequency
        elif s[0] == 'S':
            self.typ = uTYP.S  # Signal strength (SNR)
        else:
            raise ValueError
        ...
        if len(s) == 3:
            if sys == uGNSS.GPS:
                ...
                if (s[1] == '1' and s[2] not in 'IDPXSLZ') or \
                   (s[1] == '2' and s[2] not in 'IQX') or \
...
class Obs():
    ...
    def __init__(self):
        self.t = gtime_t()  # Observation time
        self.P = []         # Pseudorange observations [n_sats x n_freqs]
        self.L = []         # Carrier-phase observations [n_sats x n_freqs]
        self.S = []         # Signal-to-noise ratio (C/N0) [n_sats x n_freqs]
        self.D = []         # Doppler observations [n_sats x n_freqs]
        self.lli = []       # Loss-of-lock indicator [n_sats x n_freqs]
        self.sat = []       # List of satellite IDs
        self.sig = {}       # Dictionary of signal types per satellite system
...
class Nav():
    ...
    def __init__(self, nf=2):
        ...
        #
        self.trpModel = uTropoModel.HOPF
...
def ecef2pos(r):
    """ECEF to geodetic coordinates (latitude, longitude, height)"""
    # WGS-84 ellipsoid parameters
    a = rCST.RE_WGS84  # Semi-major axis
    e2 = rCST.FE_WGS84 * (2 - rCST.FE_WGS84)  # First eccentricity squared

    r2 = r[0]**2 + r[1]**2  # x^2 + y^2
    sqrt_r2 = np.sqrt(r2)  # sqrt(x^2 + y^2)
    z = r[2]  # Initial z
    prev_z = z
    cnt = 0
    ...
    while cnt < 1000:
        sin_phi = z / np.sqrt(r2 + z**2)  # sin(latitude)
        v = a / np.sqrt(1 - e2 * sin_phi**2)  # Prime vertical radius of curvature
        z_new = r[2] + v * e2 * sin_phi  # Updated z
        if np.abs(z - z_new) < 1e-12:  # Convergence threshold
            break
        z = z_new
        cnt += 1

    # Compute final latitude (rad), longitude (rad), and height (m)
    lat = np.arctan2(z, sqrt_r2)  # Latitude
    lon = np.arctan2(r[1], r[0])  # Longitude
    h = sqrt_r2 / np.cos(lat) - v  # Height

    # Return pos as [latitude (rad), longitude (rad), height (m)]
    pos = np.array([lat, lon, h])
    return pos

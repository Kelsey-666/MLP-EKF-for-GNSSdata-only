...
from gnss_foundation.cssrlib import sCType
from gnss_foundation.cssrlib import sCSSRTYPE as sc
import numpy as np
from gnss_foundation.gnss_type import uGNSS, rCST, sat2prn, timediff, timeadd, vnorm
from gnss_foundation.gnss_type import gtime_t, Geph, Eph, Alm, prn2sat, gpst2time, \
...
def eph2pos(t: gtime_t, eph: Eph, flg_v=False):
    """ calculate satellite position and clock based on ephemeris """
...
def satposs_enhanced(obs, nav, cs=None, orb=None):
    """
    ...
    for i in range(n):
        ...
        if nav.ephopt == 4:  # precise ephemeris
            ...
            if cs is not None:
                ...
                if 'eph' in locals() and hasattr(eph, 'tgd') and eph.tgd is not None:
                    dts[i] -= eph.tgd 
    ...
    # Remove invalid satellites (position and clock offsets are zero)
    valid_indices = [i for i in range(n)if not np.allclose(rs[i], 0.0) and not np.allclose(dts[i], 0.0)]
    ...
    return rs, vs, dts, svh, len(valid_indices)
